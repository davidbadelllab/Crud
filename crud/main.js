...
$(document).on("click", ".btnBorrar", function(){    
    ...
    if(respuesta){
        $.ajax({
            ...
            success: function(){
                tablaagentes.row(fila.parents('tr')).remove().draw();
            }
        });
    }   
});
    
$("#formagentes").submit(function(e){
    e.preventDefault();    
    ...
    $.ajax({
        ...
        type: "POST",
        dataType: "json",
        data: {nombre:nombre, usuario:usuario, contraseña:contraseña, id:id, opcion:opcion},
        success: function(data){  
            ...
            if(opcion == 1){tablaagentes.row.add([id,nombre,usuario,contraseña]).draw();}
            else{tablaagentes.row(fila).data([id,nombre,usuario,contraseña]).draw();}            
        }        
    });
